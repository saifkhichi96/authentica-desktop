...
def find_in_grid(im_grid):
    ...
    h, w = im_grid.shape
    grid_1 = im_grid[0:int(h / 2), 0:w]
    grid_2 = im_grid[int(h / 2):h, 0:w]

    # Extract all eight signatures from the grid
    _h, _w = np.array(grid_1.shape) / 2
    py, px = int(_h * 0.05), int(_w * 0.05)

    # _h, _w = int(_h - 2 * py), int(_w - 2 * px)

    signatures = []
    for x in [px, _w]:
        for y in [py, _h]:
            signatures.append((grid_1[int(y):int(y+_h), int(x):int(x+_w)], (x, y, _w, _h)))
            signatures.append((grid_2[int(y):int(y+_h), int(x):int(x+_w)], (x, h / 2 + y, _w, _h)))
    ...
    for signature, rel_boxes in signatures:
        ...
        boxes.append((int(x + _x), int(y + _y), int(w), int(h)))
...
def extract_from_check(im, model):
    ...
    x += w / 2
    y += h / 2
    return im[int(y):int(y + dy), int(x):int(x + dx)]
...
def extract_from_grid(im_grid):
    ...
    for x, y, dx, dy in boxes:
        signatures.append(im_grid[int(y):int(y + dy), int(x):int(x + dx)])
